#!/usr/bin/env python3
"""
Script de vérification des imports et modèles - Version 2
"""
import sys
import os
...
def test_imports():
    ...
    print("\n=== Test des schémas ===")
    schemas_to_test = [
        ("User schemas", "app.schemas.user", ["UserCreate", "UserOut", "UserRole", "UserUpdate"]),
        ("Schemas init", "app.schemas", [])
    ]
    
    for name, module_path, classes in schemas_to_test:
        try:
            print(f"Test {name}... ", end="")
            module = __import__(module_path, fromlist=classes)
            print("✅ OK")
...
if __name__ == "__main__":
    print("🔍 Vérification du backend ERP MIF Maroc - V2\n")
